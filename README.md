...
| Корзина | 🔴 Предложения и подтверждение реализованы, запись отключена через `UnavailableCartGateway`. | Confirm возвращает `cart_unavailable`. Нужен отдельный контракт корзины EKT. |
...
Диаграмма описывает реализованный протокол для будущего подключённого gateway. Сейчас `UnavailableCartGateway` возвращает отказ: после подтверждения товар не добавляется, интерфейс показывает это явно.
...
4. Для включения корзины всё ещё требуется отдельный контракт EKT: владелец, операции чтения/записи, идемпотентность и URL корзины. Для аналогов и условий покупки нужны утверждённые правила и источники.
