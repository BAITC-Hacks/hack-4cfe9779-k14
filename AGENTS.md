...
- The browser API contract is published at `/openapi.json` with stable operation IDs; `docs/frontend-integration.md` is the handoff guide. Configure exact browser origins with `CORS_ALLOWED_ORIGINS` (comma-separated, never `*`); `WIDGET_ORIGIN` is only the backwards-compatible single-origin fallback. The API permits `Idempotency-Key` for offer confirmation.
